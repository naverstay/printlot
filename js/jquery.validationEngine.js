...
(function($){
  ...
  var methods = {
    ...
    _validateFields        : function(form){
      ...
      if(errorFound){
        if(options.scroll){
          var destination = first_err.offset().top - 100;
    ...
    _validateField         : function(field, options, skipAjaxValidation){
      ...
      if(field.is(":hidden:not(.chosen-select)") && !options.prettySelect || field.parent().is(":hidden"))
        return false;
    ...
    _handleStatusCssClasses: function(field, options){
      var formObj = field.closest(options.parentFormClass);
      ...
      if(options.addSuccessCssClassToField){
        ...
        if(!formObj.find('.' + options.addSuccessCssClassToField).length){
      ...
      if(options.addFailureCssClassToField){
        ...
        if(!formObj.find('.' + options.addFailureCssClassToField).length){
      ...
      if(options.addSuccessCssClassToField && !options.isError){
        ...
        formObj.addClass(options.addSuccessCssClassToField);
      }

      if(options.addFailureCssClassToField && options.isError){
        field.addClass(options.addFailureCssClassToField);
        field.closest(options.parentFieldClass).addClass(options.addFailureCssClassToField);
        formObj.addClass(options.addFailureCssClassToField);
      }
    },
